...
app.get("/rest/list/", function(req, res){
    ...
    async function run() {
        try {
            ...
            const ticketDb = database.collection("415Collection");
    ...
    async function run() {
        try {
            ...
            if (ticket === null) {
                ...
                const xmlResponse = xml({ticket: ticket});
...
//A PUT request that changes XML to JSON

app.get('/putxmlform', function(req, res) {
    res.setHeader('Content-Type', 'text/html');
    fs.readFile('./put2.html', 'utf8', (err, contents) => {
        if(err) {
            console.log('Form file Read Error', err);
            res.write("<p>Form file Read Error");
        } else {
...
app.post("/rest/ticket/updateXmlTicket", function(req, res) {
    const client = new MongoClient(uri);
    ...
    async function run() {
        try {
            ...
            const xmldata = '<?xml version=”1.0" encoding=”UTF-8"?>' +
                '<ticket>' +
                '<ticketId>4</ticketId>' +
                '<type>Parking Ticket</type>' +
                '<subject>Giving this dude a ticket cause I\'m having a bad dat</subject>' +
                '<description>Parked wrong</description>' +
                '<priority>medium</priority>' +
                '<Gender>Male</Gender>' +
                '<status>open</status>' +
                '<recipient>You</recipient>' +
                '<submitter>Me</submitter>' +
                '</ticket>';

            xml2.parseString(xmldata, function (err, results) {
